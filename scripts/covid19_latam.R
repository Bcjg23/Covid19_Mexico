...
## Filtro de fechas
df_latam <- df_latam %>% filter(Date>='2020-03-2')
...
total_cases = df_latam %>% filter(Date==max(Date)) %>% select(ConfirmedCases) %>% sum()
...
total_cases = df_international %>% filter(Date=="2020-04-01") %>% select(ConfirmedCases) %>% sum()
