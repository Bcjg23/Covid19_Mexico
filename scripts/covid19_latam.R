...
df <-gather(df, `1/22/20`:`3/26/20`,key='Fecha',value=ConfirmedCases)
## Renombramos columna
...
df_latam <- df %>% filter(Country %in% c("Mexico","Colombia","Chile","Argentina","Brazil","Peru","Ecuador"))
## Filtro de fechas
df_latam <- df_latam %>% filter(Date>='2020-03-15')
## Etiquetas para el final de las series de tiempo
d_ends <- df_latam %>% group_by(Country) %>% top_n(1, ConfirmedCases) %>% pull(ConfirmedCases)
##
fecha_actualizacion <- str_sub(max(df_latam$ConfirmedCases), end = -1)

total_cases = df_latam %>% filter(Date=="2020-03-26") %>% select(ConfirmedCases) %>% sum()
...
ggplot(df_latam, aes(x = Date, y = ConfirmedCases,color = Country)) + 
  ...
  theme_ipsum()  +
  labs(title="Casos confirmados de COVID-19 en paises\n de LATAM por día", 
       subtitle=paste0("Total en estos paises: ", as.character(total_cases), "\nFecha de corte: ", df$Date[nrow(df)]),
...
df_international <- df %>% filter(Country %in% c("Mexico","Brazil","China","US","Italy","Spain","Iran","France","Korea, South"))
## Filtro de fechas
#df_international  <- df_international  %>% filter(Date>='2020-03-15')
## Etiquetas para el final de las series de tiempo
...
total_cases = df_international %>% filter(Date=="2020-03-26") %>% select(ConfirmedCases) %>% sum()


# Plot LATAM
ggplot(df_international, aes(x = Date, y = log(ConfirmedCases),color = Country)) + 
  geom_line(size = 1,alpha=0.4) +
  geom_point(size = 1)+
  #scale_y_continuous(sec.axis = sec_axis(~ ., breaks = d_ends)) + # Coloca numero de casos totales al final de la serie (lo quite por overplotting)
  theme_ipsum()  +
  labs(title="Casos confirmados de COVID-19 a nivel internacional\n por día (en escala logarítmica)", 
       subtitle=paste0("Total en estos paises: ", as.character(sum(total_cases)), "\nFecha de corte: ", df$Date[nrow(df)]),
       caption=caption_text,
       x="",
